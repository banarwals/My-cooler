...
const Sidebar = ({ user, onLogin, onLogout }: { user: FirebaseUser | null, onLogin: () => void, onLogout: () => void }) => {
  ...
  return (
    <aside className="h-screen w-64 fixed left-0 top-0 flex flex-col bg-[#0a0f13] border-r border-white/5 z-50">
...
const StatCard = ({ icon: Icon, label, value, trend, trendColor, progress }: any) => {
  return (
    <div className="bg-surface-container-low rounded-[2rem] p-7 flex items-center justify-between group hover:bg-surface-container-high transition-all duration-500">
      <div className="flex items-center gap-5">
        <div className="w-14 h-14 rounded-2xl bg-surface-variant flex items-center justify-center text-primary group-hover:scale-110 transition-transform duration-500">
          <Icon className="w-7 h-7" />
        </div>
        <div>
          <span className="text-on-surface-variant text-[10px] uppercase tracking-[0.2em] font-bold">{label}</span>
          <div className="text-2xl font-headline font-bold mt-0.5">{value}</div>
        </div>
      </div>
      {trend ? (
...
const FanDynamics = ({ speed, setSpeed, turbo, setTurbo }: any) => {
  ...
  return (
    <section className="bg-surface-container-high rounded-[2.5rem] p-8">
      <h3 className="text-sm font-headline font-bold mb-8 flex items-center gap-3 uppercase tracking-[0.2em]">
...
const SystemHealth = () => {
  ...
  return (
    <section className="bg-surface-container rounded-[2.5rem] p-8 flex-1 flex flex-col">
      <h3 className="text-sm font-headline font-bold mb-8 uppercase tracking-[0.2em]">System Health</h3>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface selection:bg-primary/30">
      ...
      <main className="ml-64 p-10 min-h-screen max-w-[1600px] mx-auto">
        <header className="flex justify-between items-end mb-12">
          <div>
            <motion.h2 
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              className="text-4xl font-black font-headline tracking-tighter text-on-surface"
            >
              AquaControl Pro
            </motion.h2>
            <p className="text-on-surface-variant text-sm font-medium mt-1 tracking-wide">
              Device ID: <span className="text-on-surface">AQ-7729-F</span> | Laboratory Environment
            </p>
          </div>
          ...
          <div className="flex items-center gap-5">
            ...
            <button 
              ...
              className={`px-5 py-2.5 rounded-full flex items-center gap-3 border transition-all ${
                btStatus === "connected" 
            ...
            {/* Online Status */}
            <div className={`px-5 py-2.5 rounded-full flex items-center gap-3 border ${
              user 
              ...
              <span className={`w-2 h-2 rounded-full ${user ? "bg-tertiary-fixed shadow-[0_0_10px_#5cfd80]" : "bg-on-surface-variant/30"}`} />
              <span className="text-[10px] font-black tracking-[0.2em] uppercase">{user ? "Cloud Sync Active" : "Cloud Offline"}</span>
            </div>
            
            <button className="w-12 h-12 flex items-center justify-center rounded-2xl bg-surface-container hover:bg-surface-container-high transition-all hover:scale-105 active:scale-95">
        ...
        <AnimatePresence>
          {error && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: -20 }}
              className="mb-8 bg-error/10 border border-error/20 p-4 rounded-2xl flex items-center justify-between"
            >
